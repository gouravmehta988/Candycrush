class Lvl2Scene extends Phaser.Scene {
  ...
  preload() {
    // Load the different objects and the background.
    ...
    this.load.image('lvl2', './assets/lvl2icon.png');
    this.load.image('bg1', './assets/lvl1bg.png');
    ...
    this.load.audio('divine', './assets/sounds/divine.mp3');
  ...
  create() {
    ...
    bgMusic.play();
    bgMusic.setVolume(0); // Start with volume 0

    let volume = 0;
    let fadeDuration = 3000; // Total time to fade in (3 seconds)
    let steps = 3; // Number of volume steps
    let stepTime = fadeDuration / steps; // Time per step

    let fadeInterval = setInterval(() => {
      volume += 0.1 / steps; // Increase volume in small steps
      bgMusic.setVolume(volume);

      if (volume >= 1) {
        clearInterval(fadeInterval); // Stop increasing when full volume is reached
      }
    }, stepTime);

    // Timer variables and display.
    // let timeRemaining = 60;
    let timerText = this.add.text(130, 130, 'Time: ' + timeRemaining2, { font: '35px bold font1', fill: 'white' });
    ...
    this.time.addEvent({
      ...
      callback: () => {
        timeRemaining2--;
        timerText.setText('Time: ' + timeRemaining2);
      },
      loop: true
    });

    // Timer: after 30 seconds, check the score and change scene accordingly.
    this.time.delayedCall(timeRemaining2*1000, () => {
      if (score >= scoreTarget2) {
        ...
        this.time.delayedCall(1000, () => {
          bgMusic.stop();
          this.scene.stop('Lvl2Scene');
          this.scene.start('Lvl3Scene');  // Move scene transition inside delay
        });
        ...
        this.time.delayedCall(2000, () => {
          bgMusic.stop();
          this.scene.start('GameOvrScene2', { score: score });
        });
      }
    }, [], this);

    // Level text and header.
    this.add.text(130, 75, 'Level 2', { font: '40px bold font1', fill: 'white' }).setDepth(3);
    ...
    // Splash screens.
    let splash = this.add.image(360, 540, 'lvl2').setDepth(10);
